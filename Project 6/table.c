 ...
 * Complexity: O(n)
 *
 * Description: The function pivot takes the last element as pivot and places the pivot element at its position in the sorted array and places the smaller pivot to the left of the pivot
 */
int pivot(SET *sp, void **data, int lo, int hi)
{
    assert(sp != NULL);
    int i, j = lo-1;
    void *pivot = data[hi];
 ...
 * Complexity: O(n)
 *
 * Description: Recursive function that runs the quicksort algorithm on elements of set pointed by sp
 */
static void quicksort(SET *sp, void **data, int lo, int hi)
{
    assert(sp != NULL);
    int loc;
    if(lo<hi)
    {
        loc = pivot(sp,data,lo,hi);
        quicksort(sp,data,lo,loc-1);
        quicksort(sp,data,loc+1,hi);
    }
    return;
}
 ...
 * Complexity: O(n)
 *
 * Description: Allocates and returns a sorted array of elements
 */
